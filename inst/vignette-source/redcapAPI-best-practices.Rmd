...
The `anytime` library in R has a robust set of date guesses that deal with a lot of different potential formats. This is a great use and example of inversion of control over type casting. Once can simply make the decision that guessing at Dates is acceptible (guessing is never the default in the redcapAPI library), and then override the defaults.
...
FIXME: Make an example with splitForms after we finish the function.
...
## Debugging Raw Connection

The library from 2.7.3 up supports `httr` debugging. 

```{r, echo=TRUE, message=TRUE, warning=TRUE, error=TRUE }
library(httr)
capture.output(
with_config(verbose(data_out=FALSE, data_in=TRUE, ssl=TRUE),
            { test <- exportRecordsTyped(test_conn, 
                              records = 10:20, 
                              forms = "text_fields") }),
type="message")
```
