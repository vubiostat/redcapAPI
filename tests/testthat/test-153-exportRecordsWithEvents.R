context("Export Records Functionality with Events")
...
test_that(
  ...
  {
    Records <- exportRecords(rcon, events = "event_1_arm_1")
...
test_that(
  "Event names can be labelled or raw", 
  {
    Records <- exportRecords(rcon, 
                             raw_or_label = "label")
    expect_true(all(Records$redcap_event_name %in% c("Event 1 (Arm 1: Arm 1)", "Event 2 (Arm 2: Arm2)")))
    
    Records <- exportRecords(rcon, raw_or_label = "raw")
    expect_true(all(Records$redcap_event_name %in% c("event_1_arm_1", "event_2_arm_2")))
  }
)
