context("Export Survey Return Code Argument Validation")

test_that(
  "Return an error if rcon is not a redcapConnection", 
  {
    expect_error(exportSurveyReturnCode(rcon = "not an rcon", 
                                  record = 1, 
                                  instrument = "survey"), 
                 "no applicable method for 'exportSurveyReturnCode'")
  }
)
...
test_that(
  ...
  {
    expect_error(exportSurveyReturnCode(rcon, 
                                  record = c("1", "2"), 
                                  instrument = "survey"), 
